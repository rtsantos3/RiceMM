...
for(i in 1:6){
  ...
  resmatrix <- calculate_raftery_stat(df)$resmatrix #Remove all chains with NAs
...
for(i in 1:6){
  ...
  Nfails <- as_data_frame('rxn'=which(resmatrix), resmatrix) %>% subset(., subset=N>5000) #N sample fail 
  Ifails <- as_data_frame(resmatrix) %>% subset(., subset=I>5) #Stickiness fail
  ...
  writeData(wb, sample, resmatrix)
  

}
workbook_file <- paste('./converge_stats/resmatrix.xlsx')
saveWorkbook(wb, wokrbook_file, overwrite=TRUE)
...
for(i in 1:6){
  ...
  file_name_resmatrix = paste('./converge_stats/resmatrix_',sample,'.csv')
